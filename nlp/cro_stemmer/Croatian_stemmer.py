...
if __name__=='__main__':
	...
	pravila=[re.compile(r'^('+osnova+')('+nastavak+r')$') for osnova, nastavak in [e.decode('utf8').strip().split(' ') for e in open(os.path.join(os.path.dirname(__file__), 'rules.txt'))]]
	transformacije=[e.decode('utf8').strip().split('\t') for e in open(os.path.join(os.path.dirname(__file__), 'transformations.txt'))]
